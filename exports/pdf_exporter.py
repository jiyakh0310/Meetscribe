...
def export_to_pdf(
    ...
    *,
    ...
    document.generated_on = _generated_on_text(meeting_info)
...
def _generated_on_text(meeting_info: dict[str, str] | None = None) -> str:
    """Return the report generation timestamp for footer presentation."""

    if meeting_info and meeting_info.get("Generated On"):
        return str(meeting_info["Generated On"])
...
def _add_mom_title_block(
    ...
    *,
    ...
    story.append(Paragraph("MeetScribe | Minutes of Meeting", styles["title"]))
...
def _add_concise_key_discussion_points(
    ...
    for item in points:
        ...
        timestamp = _escape(item.get("timestamp") or "Not Mentioned")
        speakers = _escape(", ".join(item.get("speakers") or []) or "Not Mentioned")
...
def _add_action_items(
    ...
    for item in action_items:
        rows.append([
            ...
            Paragraph(_escape(_display_due_date(item.get("due_date"))), styles["body"]),
            Paragraph(_escape(_priority_for_action_item(item)), styles["body"]),
        ])

    story.append(_table(
        rows,
        [2.95 * inch, 1.25 * inch, 1.25 * inch, 1.05 * inch],
    ))
...
def _add_next_meeting(
    ...
    *,
    ...
    if not next_meeting:
        return
    _add_heading(story, styles, "NEXT MEETING")
...
def _draw_footer(canvas: Any, document: SimpleDocTemplate) -> None:
    ...
    generated_on = str(getattr(document, "generated_on", "") or _generated_on_text())
    generated_on = generated_on.replace("\n", " | ")
    footer_text = f"Generated by MeetScribe | {generated_on} | Page {document.page}"
...
def _next_step_sentence(item: dict[str, Any]) -> str:
    ...
    if due_date and str(due_date).strip().upper() not in {"N/A", "NOT MENTIONED", "TO BE DECIDED"}:
...
def _priority_for_action_item(item: dict[str, Any]) -> str:
    ...
    if any(
        token in text
        for token in ("next sprint", "this week", "upcoming release", "soon", "planned")
    ):
        return "Medium"
    return "Low"


def _display_due_date(value: Any) -> str:
