"""Temporary local smoke test for Minutes of Meeting DOCX export.

Usage:
    python temp_test_docx_export.py
"""
...
def main() -> int:
    mom_path = export_to_docx(
        build_sample_analysis(),
        meeting_info={"Source File": "sample.aac"},
    )
    transcript_path = export_transcript_to_docx(
        build_sample_analysis(),
        meeting_info={"Source File": "sample.aac"},
    )
    print(f"Generated MoM DOCX: {mom_path}")
    print(f"Generated Transcript DOCX: {transcript_path}")
    return 0
